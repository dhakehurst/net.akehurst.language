...
export class AglEditorAce {
    ...
    constructor(
        ...
        let options = {};
        this.editor = new Editor(new AglRenderer(this.element, null), doc, options);
    ...
    tryParse() {
        if (this.processor) {
            try {
                ...
                this.updateSyntax();
                console.error("Error parsing text in " + this.languageId, e.message)
            }
        }
    }
    ...
    setStyle(css: string) {
        ...
        const module = {cssClass: this.languageId, cssText: mappedCss, _v:Date.now()};
