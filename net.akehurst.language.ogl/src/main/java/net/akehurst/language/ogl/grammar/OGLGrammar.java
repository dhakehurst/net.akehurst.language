 ...
 *   normalRule : IDENTIFIER ':' ruleItem ';' ;
 *   skipRule : 'skip' IDENTIFIER ':' choice ';' ;
 *   ruleItem : simpleChoice < priorityChoice ;
 *   simpleChoice : (concatenation / '|')* ;
 *   priorityChoice : (concatenation / '<')* ;
 *   concatenation : item+ ;
 *   item : singleItem | multi | separatedList ;
 *   singleItem : terminal | nonTerminal | group ;
 *   multiplicity : '*' | '+' | '?'
 *   multi : singleItem multiplicity ;
 *   group : '(' ruleItem ')' ;
 *   separatedList : '(' singleItem '/' terminal ')' multiplicity ;
...
public class OGLGrammar extends Grammar {
	...
	static List<Rule> createRules() {
		...
		b.rule("skipRule").concatenation( new TerminalLiteral("skip"), new NonTerminal("IDENTIFIER"), new TerminalLiteral(":"), new NonTerminal("ruleItem"), new TerminalLiteral(";") );
		b.rule("normalRule").concatenation( new NonTerminal("IDENTIFIER"), new TerminalLiteral(":"), new NonTerminal("ruleItem"), new TerminalLiteral(";") );
		b.rule("ruleItem").priorityChoice(new NonTerminal("simpleChoice"), new NonTerminal("priorityChoice") );
		...
		b.rule("concatenation").multi(1,-1,new NonTerminal("item") );
		b.rule("item").choice(
				new NonTerminal("singleItem"),
				new NonTerminal("multi"),
				new NonTerminal("separatedList")
		);
		b.rule("singleItem").choice(
				new NonTerminal("terminal"),
				new NonTerminal("nonTerminal"),
				new NonTerminal("group")
		);
		b.rule("multi").concatenation( new NonTerminal("singleItem"), new NonTerminal("multiplicity") );
		b.rule("multiplicity").choice(new TerminalLiteral("*"), new TerminalLiteral("+"), new TerminalLiteral("?"));
		b.rule("group").concatenation( new TerminalLiteral("("), new NonTerminal("ruleItem"), new TerminalLiteral(")") );
		b.rule("separatedList").concatenation( new TerminalLiteral("("), new NonTerminal("singleItem"), new TerminalLiteral("/"), new NonTerminal("LITERAL"), new TerminalLiteral(")"), new NonTerminal("multiplicity") );
