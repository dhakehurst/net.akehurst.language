...
public class Parser_Expressions_Test extends AbstractParser_Test {
	...
	public void emp_S_a() {
		...
		try {
			...
			Assert.assertEquals("{*S 1, 2}",st); //the tree is marked as if it can still grow because the top rule is multi(1-3)
	...
	public void emp_S_apa() {
		...
		try {
			...
			ToStringVisitor v = new ToStringVisitor("","");
			String st = tree.accept(v, "");
			Assert.assertEquals("{*S 1, 4}",st); //the tree is marked as if it can still grow because the top rule is multi(1-3)
