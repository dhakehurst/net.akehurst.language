...
internal class BuildCacheLC1(
    ...
    internal fun stateInfo1(): Set<StateInfo> {
        data class RpToDo(
        ...
        while (todo.isNotEmpty) {
            ...
            if (null != rule) {
                ...
                for (childRp in ruleRps) {
                    val childRpState = possibleStates[childRp] ?: PossibleState(firstOfCache, childRp)
                    val childParentFirstOfNext = when {
                        childRpState.isAtEnd -> stateToDo.parentFirstOfNext
                        else -> stateFirstOfNext
                    }
                    val childRpPrev = prevForChildren
                    val d = Pair(childRpState, childRpPrev)
                    if (done.contains(d).not()) {
                        ...
                        if (childRp.isAtStart.not()) {
                            ...
                            possibleStates[childRp] = childRpState
                        }
                        todo.enqueue(RpToDo(childRpState, childParentFirstOfNext, childRpPrev, childRpPrevForChildren, firstOfNext))
                    } else {
                        //do not recurse
                    }
                }
    ...
    internal fun stateInfo2(): Set<StateInfo> {
        ...
        class L1State(
            ...
            val prevState:L1State get() = when {
            ...
            val parentFollowAtEnd = this.parent?.followAtEnd ?: LookaheadSetPart.EMPTY //parent is null for G
            val follow = when {
                rulePosition.isAtEnd -> followAtEnd
                rulePosition.isGoal -> LookaheadSetPart.UP
                else -> rulePosition.next().map { nx ->
                    when {
                        nx.isAtEnd -> followAtEnd
                        nx.isGoal -> LookaheadSetPart.UP
                        else -> firstOf(nx, followAtEnd)
                    }
                }.reduce { acc, l -> acc.union(l) }
            }
                ...
                return when {
                    ...
                    rulePosition.isAtEnd -> {
                        ...
                        val to = parent!!.rulePosition.next().map { t ->
                            val grd = firstOf(t, parent!!.follow)
                        ...
                        val to = tgts.flatMap { t ->
                            ...
                            val ls = tParent!!.map { tp -> tp.follow }
        ...
        val startState = L1State(null, startRP, LookaheadSetPart.EMPTY)
        val finishRP = this.stateSet.finishRulePosition
        val finishState = L1State(null, finishRP, LookaheadSetPart.EMPTY)
    ...
    private fun calcHeightOrGraftInto(prev: List<RulePosition>, from: List<RuntimeRule>, upCls: Set<ClosureItemLC1>): Set<HeightGraftInfo> {
        val info = prev.flatMap { pr ->
            ...
            parentsOfFrom.flatMap { (parentPrev, parentRulePosition) ->
                val action = when {
                    parentRulePosition.isGoal-> Transition.ParseAction.GOAL
                    parentRulePosition.isAtStart -> Transition.ParseAction.HEIGHT
                    else -> Transition.ParseAction.GRAFT
                }
                ...
                targets.map { tgt ->
                    ...
                    val parentFollowAtEnd = this.firstFollowCache.followInContext(parentPrev, tgt.runtimeRule)
                    val grd2 = LookaheadSetPart.createFromRuntimeRules(parentFirstOf.toSet())
                    val up2 = LookaheadSetPart.createFromRuntimeRules(parentFollowAtEnd.toSet())
                    HeightGraftInfo(action, listOf(parentRulePosition), listOf(tgt), setOf(LookaheadInfoPart(grd2, up2)))
                }
            }
        }.toSet()
