...
//            parent.startPosition == child.startPosition && child.state.firstRule.isEmptyTerminal -> 1
   ...
   */
     ...
     */
    private fun addGrowingHead(oldHead: Set<GrowingNodeIndex>, next: GrowingNodeIndex) {
    ...
    fun growNextChild(
        ...
        return if (newParent.isComplete) {
            ...
            if (null != existingComplete) {
                 ...
                 */
                if (buildSPPT) this.treeData.setNextChildForCompleteParent(previous, newParent.complete, child, false) //addNewPreferredTreeData(previous, newParent, head)
                createNewHeadAndKeepExisting(newParent, prevPrev)
                true
            }
        } else {
