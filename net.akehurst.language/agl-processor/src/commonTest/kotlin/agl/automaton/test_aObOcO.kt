...
internal class test_aObOcO : test_AutomatonAbstract() {
    ...
    fun buildFor() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", 0, false) {
            ...
            transition(s0, s4, s5, HEIGHT, setOf(UP,b,c), setOf(setOf(UP)), listOf( RP(S, 0, SOR)))
            transition(s0, s5, s6, WIDTH, setOf(UP,c), setOf(), null)
            transition(s0, s5, s7, WIDTH, setOf(UP,c), setOf(), null)
            transition(s5, s6, s8, HEIGHT, setOf(UP,c), setOf(setOf(UP,c)), listOf( RP(bOpt, 0, SOR)))
            transition(s5, s7, s9, HEIGHT, setOf(UP,c), setOf(setOf(UP,c)), listOf( RP(bOpt, 1, SOR)))
            ...
            transition(s10, s11, s13, HEIGHT, setOf(UP), setOf(setOf(UP)), listOf( RP(cOpt, 0, SOR)))
            transition(s10, s12, s14, HEIGHT, setOf(UP), setOf(setOf(UP)), listOf( RP(cOpt, 1, SOR)))
