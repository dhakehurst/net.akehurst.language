...
internal class test_ma_sList_root_choicePriority : test_AutomatonAbstract() {
    ...
    fun buildFor() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", false) {
            ...
            transition(HEIGHT) { ctx(RP(G, o0, SOR), RP(rA, OLI, PLI), RP(rM, OLI, PLI)); src(rA, OLI, ER); tgt(E, o2, ER); lhg(setOf(EOT, a, m), setOf(EOT, a, m)) }
            ...
            transition(HEIGHT) { ctx(G, o0, SOR); src(E, o0, ER); tgt(S); lhg(setOf(EOT), setOf(EOT)) }
            transition(HEIGHT) { ctx(G, o0, SOR); src(E, o1, ER); tgt(S); lhg(setOf(EOT), setOf(EOT)) }
            transition(HEIGHT) { ctx(G, o0, SOR); src(E, o2, ER); tgt(S); lhg(setOf(EOT), setOf(EOT)) }
        }
