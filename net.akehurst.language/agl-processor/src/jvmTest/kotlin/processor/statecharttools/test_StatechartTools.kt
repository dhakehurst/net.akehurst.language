...
class test_StatechartTools {
    ...
    data class Data(val file: String, val grammarName:String, val ruleName: String, val text: String) {
        override fun toString(): String  = "${this.file} | ${this.grammarName} | ${this.ruleName} | ${this.text}"
    }

    @ParameterizedTest(name = "{index} {0}")
    @MethodSource("collectData")
    fun parse(data: Data) {
        val result = processor[data.grammarName]!!.parse(data.text, Agl.parseOptions { goalRuleName(data.ruleName) })
    ...
    fun process(data: Data) {
        val result = processor[data.grammarName]!!.process(data.text, Agl.options { parse { goalRuleName(data.ruleName) } })
