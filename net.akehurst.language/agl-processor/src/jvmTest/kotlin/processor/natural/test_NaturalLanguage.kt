...
@Ignore // lots of syntax errors, grammar needs fixing
class test_NaturalLanguage {
    ...
    @MethodSource("data")
    fun test(data: Data) {
        val sentence = SentenceDefault(data.sentence, null)
        ...
        val result = processor.parse(data.sentence, Agl.parseOptions { goalRuleName(data.goal) })
        assertNotNull(result.sppt, result.issues.toString())
        assertTrue(result.issues.errors.isEmpty(), result.issues.toString())

        val resultStr = result.sppt!!.asSentence
        assertEquals(data.sentence, resultStr)
