...
object TypeModelTest {
    ...
    private fun tmAssertEquals(expected: DataType, actual: DataType) {
        ...
        for (i in expected.property.indices) {
            ...
            assertNotNull(actEl, "expected PropertyDeclaration '${expEl}' not found in actual ElementType '${expected.name}'. [${actual.property.joinToString { it.name }}]")
    ...
    private fun tmAssertEquals(expected: TupleType, actual: TupleType) {
        ...
        for (i in expected.property.indices) {
            val expEl = expected.property[i]
            val actEl = actual.property[i]
            assertNotNull(actEl, "expected PropertyDeclaration '$expEl' not found in actual TupleType '${expected.name}'. [${actual.property.joinToString { it.name }}]")
            tmAssertEquals(expEl, actEl)
        }
    }
