...
class test_sList_followed_by_separator_literal : test_LeftCornerParserAbstract() {

    private companion object {
        val rrs = runtimeRuleSet {
            concatenation("S") { ref("sList"); literal(";")  }
            sList("sList", 0, -1, "'a'", "';'")
            literal("a")
            literal(";")
        }
    ...
    fun semi() {
        ...
        val expected = "S { sList{<EMPTY_LIST>} ';' }"
    ...
    fun a_semi_a__fails() {
        ...
        assertEquals(
            listOf(
                parseError(InputLocation(3, 4, 1, 1, null), sentence, setOf("sList"), setOf("';'"))
            ), issues.errors
        )
    }
