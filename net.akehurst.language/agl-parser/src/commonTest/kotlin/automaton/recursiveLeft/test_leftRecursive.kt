...
class test_leftRecursive : test_AutomatonAbstract() {
    ...
    fun parse_a() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", false) {
            ...
            trans(HEIGHT) { src(a); tgt(S); lhg(setOf(EOT, a), setOf(EOT, a)); ctx(rG, oN, SOR) }
            trans(HEIGHT) { src(S); tgt(S1, oN, p1); lhg(setOf(a), setOf(EOT, a)); ctx(rG, oN, SOR) }
        }
    ...
    fun parse_aa() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", false) {
            ...
            trans(WIDTH) { ctx(rG, oN, SOR); src(rG, oN, SOR); tgt(a); lhg(setOf(EOT, a)) }
            trans(WIDTH) { ctx(rG, oN, SOR); src(S1, oN, p1); tgt(a); lhg(setOf(RT)) }
            trans(GOAL) { ctx(rG, oN, SOR); src(S); tgt(rG); lhg(setOf(EOT)) }
            trans(GOAL) { ctx(rG, oN, SOR); src(S, o1, EOR); tgt(rG); lhg(setOf(EOT)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(a); tgt(S); lhg(setOf(EOT, a), setOf(EOT, a)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(S1); tgt(S, o1, ER); lhg(setOf(RT, a), setOf(RT, a)) }
            trans(GRAFT) { ctx(S1, oN, p1); src(a); tgt(S1); lhg(setOf(RT)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(S); tgt(S1, oN, p1); lhg(setOf(a), setOf(EOT, a)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(S, o1, ER); tgt(S1, oN, p1); lhg(setOf(a), setOf(RT, a)) }
        }
        AutomatonTest.assertEquals(expected, actual)
    }
    ...
    fun parse_aaa() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", false) {
            ...
            trans(WIDTH) { ctx(rG, oN, SOR); src(S1, oN, p1); tgt(a); lhg(setOf(RT)) }
            trans(GOAL) { ctx(rG, oN, SOR); src(S); tgt(rG); lhg(setOf(EOT)) }
            trans(GOAL) { ctx(rG, oN, SOR); src(S, o1, ER); tgt(rG); lhg(setOf(EOT)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(a); tgt(S); lhg(setOf(EOT, a), setOf(EOT, a)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(S1); tgt(S, o1, ER); lhg(setOf(RT, a), setOf(RT, a)) }
            trans(GRAFT) { ctx(S1, oN, p1); src(a); tgt(S1); lhg(setOf(RT)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(S); tgt(S1, oN, p1); lhg(setOf(a), setOf(EOT, a)) }
    ...
    fun buildFor() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", false) {
            ...
            trans(WIDTH) { ctx(rG, oN, SOR); src(rG, oN, SOR); tgt(a); lhg(setOf(EOT, a)) }
            trans(WIDTH) { ctx(rG, oN, SOR); src(S1, oN, p1); tgt(a); lhg(setOf(EOT, a)) }
            trans(GOAL) { ctx(rG, oN, SOR); src(S); tgt(rG); lhg(setOf(EOT)) }
            trans(GOAL) { ctx(rG, oN, SOR); src(S, o1, ER); tgt(rG); lhg(setOf(EOT)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(a); tgt(S); lhg(setOf(EOT, a), setOf(EOT, a)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(S1); tgt(S, o1, ER); lhg(setOf(EOT, a), setOf(EOT, a)) }
            trans(GRAFT) { ctx(S1, oN, p1); src(a); tgt(S1); lhg(setOf(EOT, a)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(S); tgt(S1, oN, p1); lhg(setOf(a), setOf(EOT, a)) }
            trans(HEIGHT) { ctx(rG, oN, SOR); src(S, o1, ER); tgt(S1, oN, p1); lhg(setOf(a), setOf(EOT, a)) }
        }
