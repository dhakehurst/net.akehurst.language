...
class test_aObOcO : test_AutomatonAbstract() {
    ...
    fun parse_a() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", false) {
            ...
            trans(WIDTH) { src(G, oN, SOR); tgt(EMPTY_LIST); lhg(setOf(EOT, b, c)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(S, oN, p1); tgt(EMPTY_LIST); lhg(setOf(RT, c)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(S, oN, p2); tgt(EMPTY_LIST); lhg(setOf(RT)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(G, oN, SOR); tgt(a); lhg(setOf(EOT, b, c)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(S, oN, p1); tgt(b); lhg(setOf(RT, c)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(S, oN, p2); tgt(c); lhg(setOf(RT)); ctx(G, oN, SOR) }
            trans(GOAL) { src(S);tgt(G); lhg(EOT); ctx(G, oN, SOR) }
            trans(GRAFT) { src(cOpt, oLE, EOR); tgt(S); lhg(RT); ctx(S, oN, p2) }
            trans(HEIGHT) { src(aOpt, oLI, EOR); tgt(S, oN, p1); lhg(setOf(EOT, b, c), setOf(EOT)); ctx(G, oN, SOR) }
            trans(GRAFT) { src(bOpt, oLE, EOR); tgt(S, oN, p2); lhg(setOf(RT, c)); ctx(S, oN, p1) }
            trans(HEIGHT) { src(a); tgt(aOpt, oLI, EOR); lhg(setOf(EOT, b, c), setOf(EOT, b, c)); ctx(G, oN, SOR) }
    ...
    fun parse_b() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", false) {
            ...
            trans(GOAL) { src(S); tgt(G); lhg(EOT); ctx(G, oN, SOR) }
            trans(GRAFT) { src(cOpt, oLE, EOR); tgt(S); lhg(RT); ctx(S, oN, p2) }
            trans(HEIGHT) { src(aOpt, oLE, EOR); tgt(S, oN, p1); lhg(setOf(EOT, b, c), setOf(EOT)); ctx(G, oN, SOR) }
            trans(GRAFT) { src(bOpt, oLI, EOR); tgt(S, oN, p2); lhg(setOf(RT, c)); ctx(S, oN, p1) }
            trans(HEIGHT) { src(EMPTY_LIST); tgt(aOpt, oLE, EOR); lhg(setOf(EOT, b, c), setOf(EOT, b, c)); ctx(G, oN, SOR) }
    ...
    fun buildFor() {
        ...
        val expected = automaton(rrs, AutomatonKind.LOOKAHEAD_1, "S", false) {
            ...
            trans(WIDTH) { src(G, oN, SOR); tgt(EMPTY_LIST); lhg(setOf(EOT, b, c)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(S, oN, p1); tgt(EMPTY_LIST); lhg(setOf(EOT, c)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(S, oN, p2); tgt(EMPTY_LIST); lhg(setOf(EOT)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(G, oN, SOR); tgt(a); lhg(setOf(EOT, b, c)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(S, oN, p1); tgt(b); lhg(setOf(EOT, c)); ctx(G, oN, SOR) }
            trans(WIDTH) { src(S, oN, p2); tgt(c); lhg(setOf(EOT)); ctx(G, oN, SOR) }
            trans(GOAL) { src(S); tgt(G); lhg(EOT); ctx(G, oN, SOR) }
