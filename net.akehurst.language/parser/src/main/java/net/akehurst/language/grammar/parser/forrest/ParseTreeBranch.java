...
public class ParseTreeBranch extends AbstractParseTree {
	...
	boolean calculateIsComplete() {
		switch (this.rule.getRhs().getKind()) {
			...
			case CONCATENATION: {
				return this.rule.getRhs().getItems().length <= this.nextItemIndex || this.nextItemIndex == -1; // the -1 is used when creating dummy
																												// branch...should really need the test here!
			}
			case MULTI: {
				int size = this.nextItemIndex;
				return size >= this.rule.getRhs().getMultiMin() || this.nextItemIndex == -1; // the -1 is used when creating dummy branch...should really need
																								// the test here!
			}
			case SEPARATED_LIST: {
				int size = this.nextItemIndex;
