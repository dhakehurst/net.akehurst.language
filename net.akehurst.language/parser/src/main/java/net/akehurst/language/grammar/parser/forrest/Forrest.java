...
// toString output  CG<name> (a,b,c,d)
// C = * is complete, + = not complete
// G = ? can grow (nothing if can't grow)
// <name> node name
// (a,b,c,d) = (ruleNumber,start, end, nextItemIndex)
// 
...
public class Forrest {
	...
	Input input;
	HashSet<AbstractParseTree> possibleTrees = new HashSet<>();
...
//		return this.possTrees2.size();
	...
	public AbstractParseTree[] getPossibles() {
		return this.possibleTrees.toArray(new AbstractParseTree[this.possibleTrees.size()]);
//		return this.possTrees2.values().toArray(new AbstractParseTree[this.possTrees2.size()]);
	...
	Forrest newForrest() {
		...
		newForrest.cachedParses = this.cachedParses;
...
//			ArrayList<AbstractParseTree> newBranches = tree.growWidthAndHeight(this.runtimeRuleSet);
			ArrayList<AbstractParseTree> newBranches = this.getPreviousParse(tree);
			if (null==newBranches) {
				newBranches = this.growTreeWidthAndHeight(tree);
				this.cacheParse(tree, newBranches);
			} else {
				//use the cached branches
				int i=0;
			}
	...
	Map<ILeaf, ArrayList<AbstractParseTree>> cachedParses;
	ArrayList<AbstractParseTree> getPreviousParse(AbstractParseTree tree) {
		if (tree instanceof ParseTreeBud) {
			return this.cachedParses.get(tree.getRoot());
		} else {
			return null;
		}
		
	}
	void cacheParse(AbstractParseTree tree, ArrayList<AbstractParseTree> parse) {
		ILeaf leaf = tree.getRoot().getFirstLeaf();
		if (this.cachedParses.containsKey(leaf)) {
			this.cachedParses.get(leaf).addAll(parse);
		} else {
			cachedParses.put(leaf, parse);
		}
	}
...
//	int overwrites;
	...
	AbstractParseTree mergeContainedTree(AbstractParseTree tree) {
		AbstractParseTree st = tree.stackedTree;
		if(null==st) {
			//do nothing
		} else {
			AbstractParseTree merged = this.mergeContainedTree(st);
			tree.stackedTree = merged;
		}
	...
	//			AbstractParseTree old = this.containedTrees.get(tree.identifier);
				AbstractParseTree merged = this.mergeContainedTree(tree);
				if (tree==merged) {
					this.possibleTrees.add(tree);
				} else {
					//tree was merged into another stack
				}
	...
	//			}
			...
			//if no complete just add it and deal with it when it is complete
			if (tree.getHasPotential(this.runtimeRuleSet)) {
				this.possibleTrees.add(tree);
				this.canGrow |= tree.getCanGrow();
			}
...
//		clone.possTrees2 = new HashMap<>(this.possTrees2);
		...
		clone.cachedParses = cachedParses;
		return clone;
	}
