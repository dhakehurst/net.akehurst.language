...
public class ChoiceAbstractSingleOneSeparatedList extends AbstractChoice2RuntimeRuleItem<AbstractChoice> {
    ...
    public boolean isValidForLeft2Right(final AbstractChoice left) {
        return 1 == left.getAlternative().size() && 1 == left.getAlternative().get(0).getItem().size()
                && left.getAlternative().get(0).getItem().get(0) instanceof SeparatedList;
    }
    ...
    public RuntimeRuleItem constructLeft2Right(final AbstractChoice left, final BinaryTransformer transformer) {
        final SeparatedList item = (SeparatedList) left.getAlternative().get(0).getItem().get(0);
    ...
    @Override
    public void updateRight2Left(final AbstractChoice arg0, final RuntimeRuleItem arg1, final BinaryTransformer transformer) {
