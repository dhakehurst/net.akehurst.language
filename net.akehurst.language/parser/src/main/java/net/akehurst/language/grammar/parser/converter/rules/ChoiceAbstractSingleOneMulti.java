...
public class ChoiceAbstractSingleOneMulti extends AbstractChoice2RuntimeRuleItem<ChoiceAbstract> {
    ...
    public boolean isValidForLeft2Right(final ChoiceAbstract left) {
        return 1 == left.getAlternative().size() && 1 == left.getAlternative().get(0).getItem().size()
                && left.getAlternative().get(0).getItem().get(0) instanceof MultiDefault;
    }
    ...
    public RuntimeRuleItem constructLeft2Right(final ChoiceAbstract left, final BinaryTransformer transformer) {
        final MultiDefault multi = (MultiDefault) left.getAlternative().get(0).getItem().get(0);
    ...
    @Override
    public void updateRight2Left(final ChoiceAbstract left, final RuntimeRuleItem right, final BinaryTransformer transformer) {
